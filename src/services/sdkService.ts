...
/**
 * Fetch an avatar URL using the SDK access token and return a data URI.
 * Falls back to empty string if the fetch fails.
 */
async function fetchAvatarAsDataUri(avatarUrl: string): Promise<string> {
  if (!avatarUrl) return "";
  if (_avatarCache.has(avatarUrl)) return _avatarCache.get(avatarUrl)!;
  ...
  try {
    ...
    _avatarCache.set(avatarUrl, dataUri);
    return dataUri;
  } catch {
    return "";
  }
...
/**
 * Resolve all avatar URLs in a PR to data URIs so they display in the iframe.
 */
export async function resolveAvatars(pr: IPullRequestItem): Promise<IPullRequestItem> {
  const creatorAvatar = await fetchAvatarAsDataUri(pr.createdBy.imageUrl);
  const reviewerAvatars = await Promise.all(
    pr.reviewers.map((r) => fetchAvatarAsDataUri(r.imageUrl))
  );
  return {
    ...pr,
    createdBy: { ...pr.createdBy, imageUrl: creatorAvatar },
    reviewers: pr.reviewers.map((r, i) => ({ ...r, imageUrl: reviewerAvatars[i] })),
  };
}
